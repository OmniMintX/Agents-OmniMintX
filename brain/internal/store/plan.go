...
func (s *Store) CreatePlan(planID, goal, projectID string, tasks []NewTask) error {
	...
	return s.inTx(func(tx *sql.Tx) error {
		...
		for _, t := range tasks {
			if _, err := tx.Exec(
				`INSERT INTO tasks (id, plan_id, title, prompt, harness, check_cmd, verify, requires_approval, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
				t.ID, planID, t.Title, t.Prompt, t.Harness, t.Check, t.Verify, t.RequiresApproval, TaskPending, ts,
			); err != nil {
...
func (s *Store) GetTasks(planID string) ([]Task, error) {
	rows, err := s.db.Query(
		`SELECT id, plan_id, title, prompt, harness, check_cmd, verify, requires_approval, status, ao_session_id, branch, pr_url, created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.ID, &t.PlanID, &t.Title, &t.Prompt, &t.Harness, &t.Check, &t.Verify, &t.RequiresApproval, &t.Status, &sess, &branch, &pr, &created); err != nil {
			return nil, err
		}
