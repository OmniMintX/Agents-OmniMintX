...
- **Wave 3**: OM-11 **DONE** (0e1f417 aoclient config Get/Update + 6e241ce autonomy knob/ensureAutonomy; E2E lần 6) — **zero-touch ĐẠT với `bypass-permissions`** (run 3, sau opt-in `autonomy_allow_bypass`: 0 prompt, plan done ~90s); `auto` và `accept-edits` KHÔNG zero-touch được (classifier của `auto` chặn cả lệnh vô hại như `git add`/`git config`; `accept-edits` hỏi mọi bash compound). Cơ chế set permission PASS, trust-dialog đã biến mất. OM-12 **DONE** (e60947b store/planner + a66bae8 scheduler gate/CLI/notifier; E2E lần 7 PASS cả 4 kịch bản sống) — gate chặn thật trước dispatch (không AO session nào tạo cho task chưa approve, kể cả qua kill-resume), approve/reject từ process khác hiệu lực ≤ 1 tick, reject cascade `dependency_failed` đúng dependents và không lây nhánh độc lập, notification osascript vào Notification Center ở cả 3 sự kiện (bằng chứng unified log, 6/6 khớp events). Chi tiết brain/docs/e2e.md lần 7.
- **Wave 4** (OM-14) chưa bắt đầu.
- **Bug phát hiện E2E lần 6 — ĐÃ vá + validate sống**: planner sinh check tier-0 đòi `git status --porcelain` sạch, nhưng marker protocol bắt worker để `.om-done.<hex8>` uncommitted → check fail 100% mọi round → `verify_budget_exhausted` oan. Fix: `gitops.EnsureExcluded` ghi `.om-done.*` vào `<repo>/.git/info/exclude` lúc run start (gọi trong `Scheduler.Run` sau `resolveRepo`; git common dir → áp dụng mọi linked worktree), kèm dặn planner không sinh check assert worktree sạch. Re-run E2E với `bypass-permissions` (run 3) ĐÃ XONG: OM-11c PASS zero-touch, fix xác nhận sống (`git status --porcelain` sạch sau run). Chi tiết brain/docs/e2e.md lần 6.

#### OM-12 — những gì đã hiện thực
- **Gate trước dispatch, event-sourced**: task `requires_approval` đến lượt dispatch → scheduler (gác duy nhất, trong `dispatchReady`) ghi `task_approval_requested` MỘT LẦN → derived status `awaiting_approval` (không đốt slot MaxParallel, không có timeout clock, KHÔNG tạo AO session); `om run` sống chờ. Idempotent qua derive: awaiting rời ready-list nên resume/replay không ghi event trùng, không re-notify.
- **CLI**: `om approve-task <plan> <task>` (hoặc `--all` — approve đúng những task đang awaiting tại thời điểm gọi) ghi `task_approved` → tick sau dispatch (đo sống: 1–13s với poll 15s); `om reject-task <plan> <task> --reason` ghi `task_failed kind=rejected` (chỉ nhận từ awaiting_approval) → scheduler cascade `task_failed kind=dependency_failed` lên dependents (transitively), nhánh độc lập không lây. Approve từ process khác/khi run không chạy đều hợp lệ (event nằm trong DB, resume nhặt). Approve MỘT LẦN là vĩnh viễn — retry round OM-10 không re-gate.
- **Ai gán cờ**: planner LLM per-task qua prompt rule (destructive/irreversible/production → `requires_approval: true`); E2E lần 7 planner gán đúng 3/3 plan từ wording goal. Override tay qua `om plan --edit`. Cột DB `tasks.requires_approval`.
- **Notifier** (interface trong scheduler, impl `cmd/om`): bắn ở 3 sự kiện — task vào awaiting_approval, task vào needs_human, plan done/failed. Knob `notify: auto|bell|off` (env `OVERMIND_NOTIFY`, flag `--notify`); `auto` trên darwin = osascript `display notification` (escape `\` `"`; lỗi → fallback bell + warning), OS khác/`bell` = terminal bell; best-effort, không bao giờ fail run.
- **3 follow-up non-blocking đã ghi nhận, chưa vá**: (1) `task_dispatching` "giả" trên dependent trong cascade (quan sát sống E2E lần 7 — FailTask không có transition pending→failed); (2) TOCTOU nhỏ ở reject-task (có thể reject task vừa approve+dispatch); (3) osascript gặp newline trong title/body → rơi bell fallback. Divergence nhỏ: `om plan` chưa in cột APPROVAL (xem qua `om status`/DB).
...
- OM-10 (verifier LLM + retry budget) đã xong và merge (45c6fa5 + 76f74f4); OM-11 (autonomy, default `auto`) DONE (0e1f417 + 6e241ce) — OM-11c PASS zero-touch với `bypass-permissions` (E2E lần 6 run 3); bug planner-check-vs-marker đã vá + validate sống (xem Trạng thái hiện tại); OM-12 (approval gates + notification) DONE (e60947b + a66bae8) — E2E lần 7 PASS cả 4 kịch bản; việc kế tiếp là OM-14 (Wave 4, E2E Phase 2) để đóng Phase 2.
