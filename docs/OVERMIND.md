...
- OM-10 (verifier LLM + retry budget) đã xong và merge (45c6fa5 + 76f74f4); OM-11 (autonomy, default `auto`) DONE (0e1f417 + 6e241ce) — OM-11c PASS zero-touch với `bypass-permissions` (E2E lần 6 run 3); bug planner-check-vs-marker đã vá + validate sống (xem Trạng thái hiện tại); việc kế tiếp là OM-12 (approval gates).
